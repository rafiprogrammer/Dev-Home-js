...
completeBtn.forEach((complete) => {
  complete.addEventListener("click", function (event) {
    ...
    let title = event.target.parentElement.parentElement.parentElement.querySelector('.my-4.text-2xl');
    console.log(title);
    ...
    div.innerHTML = `<p>You have completed the task ${title.innerText} at ${currentTime}</p>`;
    activitySection.appendChild(div);
    

    // Clear Activity Section
    document
      .getElementById("clear-text")
